...
(function (global) {
	...
	function defineFidoo() {
		...
		// signature-matching bug. Callers such as fidoo-web.html should await
		// this before allowing the user to identify a file.
		...
		Fidoo.loadSignatureData = function (which) {
			...
			if (which === "browser") {
				...
				for (const signatureFile in Fidoo.signatureMapJSON) {
					...
					// json/*.js and the <head> of fidoo-web.html) over an XHR
					// fetch. Browsers treat a file:// page as a "null" origin
		...
		Fidoo.loadJSON = function (signatureFile, urlJSON) {
			...
			// file), which breaks the common "just open fidoo-web.html from
			// disk, no server needed" workflow. XHR still supports file:// in
			// the browsers this demo targets.
