...
// 					Fidoo.consoleFatal(message); // Chrome chokes
					...
					console.log(message);
// 					Fidoo.consoleDebug(message); // Chrome chokes, needs fix
					}
				}
			} // end logHandler
...
/* common functions ************************************************* */
		...
		Fidoo.readSetup = function(which) {
			if(which == "nodeJS") {
				try {
					var FidooUserSetup = require("./fidoo-setup.js");
		...
		Fidoo.loadSignatureData = function (which) {
			...
			if(which == "nodeJS") {
				try { // TODO more catching
					Fidoo.registerJSON(require(Fidoo.pathNodeJSON + Fidoo.pronomSignaturesJSON), "pronomSignatures");
					Fidoo.registerJSON(require(Fidoo.pathNodeJSON + Fidoo.extensionPuidMapJSON), "extensionPuidMap");
					Fidoo.registerJSON(require(Fidoo.pathNodeJSON + Fidoo.mimePuidMapJSON), "mimePuidMap");
					}
				catch(error) {
			...
			if(which == "browser") {
				try { // TODO more catching
					Fidoo.loadJSON(Fidoo.pathBrowserJSON + Fidoo.pronomSignaturesJSON, "pronomSignatures");
					Fidoo.loadJSON(Fidoo.pathBrowserJSON + Fidoo.extensionPuidMapJSON, "extensionPuidMap");
					Fidoo.loadJSON(Fidoo.pathBrowserJSON + Fidoo.mimePuidMapJSON, "mimePuidMap");
					}
				catch(error) {
					Fidoo.logHandler("fatal error: ", error);
					}				
