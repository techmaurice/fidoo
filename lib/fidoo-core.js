			...
			for(var i = 0, file; file = files[i]; i++) {
				...
				if(search === filename) {
					...
					fileReader.onload = function(e) {
						try {
							var result = Fidoo.identifyFile(filename, mimetype, e.target.result);
...
/* node.js functions ************************************************ */
		...
		Fidoo.identifyFilesNode = function() {
			var content = Fidoo.fs.readFileSync(Fidoo.nodeInputFile).toString();
			var files = content.trim().split("\n");
			if(files[0] === "") {
				Fidoo.logHandler("fatal error", Fidoo.nodeInputFile + " is empty");
				return false;
				}
			for(var i = 0, file; file = files[i]; i++) {
				Fidoo.identifyResults[file] = {};
				}
// 			console.log(Fidoo.identifyResults);
			} // end identifyFilesNode
		...
		Fidoo.setNodeOutputFile = function(filename) {
			// needs more catching on file touch
			Fidoo.fs.closeSync(Fidoo.fs.openSync(filename, "w"));
			Fidoo.testFileAccessNode(filename);
			Fidoo.nodeOutputFile = filename;
			} // end setOutputFileNode

		Fidoo.testFileAccessNode = function(filename) {
			Fidoo.fs.stat(filename, function(error, stat) {
				if(error == null) {
					return true;
					}
				else if(error.code == "ENOENT") {
					Fidoo.logHandler("fatal error", "File " + filename + " ENOENT error: " + error.code);
					return false;
					}
				else {
					Fidoo.logHandler("fatal error", "File " + filename + " reported " + error.code);
					return false;
					}
				});
			return false;
...
/* identification functions ***************************************** */
		...
		Fidoo.identifyFile = function(filename, mimetype, binaryString) {
...
// 			return true;
			...
			for(var puid in regexesMap) {
				for(var i = 0; i < regexesMap[puid].length; i++) {
					...
					if(regexesMap[puid][i].eofregex != null && bofResult != null) {
						...
						if(bofResult != null) {
							var name = Fidoo.pronomSignatures[puid].name ? Fidoo.pronomSignatures[puid].name : "";
							var version = Fidoo.pronomSignatures[puid].version ? Fidoo.pronomSignatures[puid].version : "";
							var pronomMimetype = Fidoo.pronomSignatures[puid].mime ? Fidoo.pronomSignatures[puid].mime : "";
							var contentType = Fidoo.pronomSignatures[puid].content_type ? Fidoo.pronomSignatures[puid].content_type : "";
							var appleUid = Fidoo.pronomSignatures[puid].apple_uid ? Fidoo.pronomSignatures[puid].apple_uid : "";
							var hasPriorityOver = Fidoo.pronomSignatures[puid].has_priority_over ? Fidoo.pronomSignatures[puid].has_priority_over : "";

							result.signatureMatches.push({
								puid: puid,
								signaturename: regexesMap[puid][i].name, 
								name: name,
								version: version,
								pronomMimetype: pronomMimetype,
			...
			result.matchingMethod = Fidoo.matchingMethod;
