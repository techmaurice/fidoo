...
(function (global) {
	...
	function defineFidoo() {
		...
		// Fidoo.ready resolves once signature data has actually finished loading
		// and regexesMap has been built. In node.js this is already true by the
		// time loadSignatureData() returns (signature files are require()'d
		// synchronously), but in a browser the JSON is fetched asynchronously —
		// calling identifyFile() before this resolves will silently see empty
		// pronomSignatures/regexesMap/extensionPuidMap/mimePuidMap and return a
		// result with no matches at all, which looks like (but is not) a
		// signature-matching bug. Callers such as fidoo-standalone.html and
		// fidoo-web-example.html should await this before allowing the user
		// to identify a file.
		Fidoo.ready = Promise.resolve();
		...
		Fidoo.loadSignatureData = function (which) {
			...
			if (which === "browser") {
				const pending = [];
				for (const signatureFile in Fidoo.signatureMapJSON) {
					const filename = Fidoo[signatureFile];
					// Prefer data preloaded via a plain <script> tag (see
					// json/*.js and the <head> of fidoo-standalone.html) over an
					// XHR fetch. Browsers treat a file:// page as a "null" origin
					// and, as of current Chrome, refuse essentially any
					// same-tree file:// request from it - not just fetch(),
					// XHR too - which breaks the "just open the HTML file"
					// workflow entirely. A <script src="..."> tag isn't
					// subject to that restriction (it's how fidoo-core.js
					// itself is loaded), so shipping the signature data the
					// same way sidesteps the problem rather than depending on
					// whichever request API browsers haven't restricted yet.
					const preloaded = typeof window !== "undefined" && window.FidooPreloadedJSON && window.FidooPreloadedJSON[filename];
					if (preloaded) {
						Fidoo.registerJSON(Fidoo.signatureMapJSON[signatureFile], preloaded);
						pending.push(Promise.resolve());
						continue;
					}
					const urlJSON = Fidoo.pathBrowserJSON + filename;
					pending.push(Fidoo.loadJSON(Fidoo.signatureMapJSON[signatureFile], urlJSON));
				}
				Fidoo.ready = Promise.all(pending).then(() => undefined);
			}
		...
		Fidoo.loadJSON = function (signatureFile, urlJSON) {
			// Deliberately XMLHttpRequest, not fetch(): fetch() refuses file://
			// URLs outright (fails with a CORS error even for a same-directory
			// file), which breaks the common "just open fidoo-standalone.html
			// from disk, no server needed" workflow. XHR still supports file://
			// in the browsers this demo targets. This fallback is what
			// fidoo-web-example.html's own fetch()-based preload sidesteps -
			// see that file for a fetch() approach that works when served over
			// an actual http(s) server instead of opened from disk.
			return new Promise((resolve, reject) => {
				...
				request.onreadystatechange = () => {
					...
					if (request.status === 200 || (request.status === 0 && request.responseText)) {
						// status 0 is what file:// requests report on success in browsers that allow them
						try {
