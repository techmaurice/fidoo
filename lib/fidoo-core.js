...
/* identification functions ***************************************** */
		...
		Fidoo.registerJSON = function(signatures, json) {
			...
			if(Object.keys(Fidoo.signatureMapJSON).length === Fidoo.checkJSON.length) {
				...
				// append signature data instead of assigning
				var temp;
				temp = Fidoo.mimeToPuidsMapping(Fidoo.pronomSignatures);
				for(var key in temp) {
					Fidoo.mimePuidMap[key] = temp[key];
					}	
				temp = Fidoo.extensionToPuidsMapping(Fidoo.pronomSignatures);
				for(var key in temp) {
		...
		Fidoo.mimeToPuidsMapping = function(formats) {
			var mimeMap = new Object;
			for (var format in formats) {
				if (formats.hasOwnProperty(format)) {
					if(Fidoo.isArray(formats[format].mime)) {
						for(var x = 0; x < formats[format].mime.length; x++) {
			...
			return mimeMap;
			} // end mimeToPuidsMapping

		Fidoo.extensionToPuidsMapping = function(formats) {
			var extMap = new Object;
			for (var format in formats) {
				if (formats.hasOwnProperty(format)) {
					if(Fidoo.isArray(formats[format].extension)) {
						for(var x = 0; x < formats[format].extension.length; x++) {
